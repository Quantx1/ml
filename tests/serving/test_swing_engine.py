...
from backend.ai.signals.engines.swing import SwingEngine
from backend.ai.signals.style_types import SwingSignal
from ml.data.benchmark import load_nifty_benchmark
from ml.features.forecast_features import FORECAST_FEATURES
...
def _trading_dates(n: int = 400) -> pd.DatetimeIndex:
    ...
    SwingEngine loads its RS benchmark via load_nifty_benchmark (the real
    ...
    dates include NSE holidays, which NaN out the 63-bar beta/corr windows
    and drop every row.
    """
...
def test_ranks_and_levels_and_outputs():
    syms = ["AAA", "BBB", "CCC"]
    # NSEI offset kept for shape parity with the momentum test, but the swing
    # benchmark comes from load_nifty_benchmark — the loader only serves the
    # equity universe.
    offsets = {"AAA": 0.0, "BBB": 5.0, "CCC": 10.0, "NSEI": 3.0}
    ...
    eng = SwingEngine(_booster=_FakeBooster(), _feature_order=None,
                      _universe=lambda limit=None: syms, _load_ohlcv=fake_loader)
    sigs = eng.run(top_n=3)
    assert len(sigs) == 3
    assert all(isinstance(s, SwingSignal) for s in sigs)
...
def test_xs_rank_features_are_cross_sectional_not_degenerate():
    ...
    eng = SwingEngine(_booster=cap, _feature_order=None,
                      _universe=lambda limit=None: syms, _load_ohlcv=fake_loader)
    eng.run(top_n=3)
    xs_vals = [df["xs_rank_ret_10"].iloc[0] for df in cap.seen]
    assert len(xs_vals) >= 2
    assert len({round(v, 6) for v in xs_vals}) > 1, \
        "xs_rank_ret_10 is degenerate across symbols — features built per-symbol, not cross-sectionally"


def test_honest_empty_when_model_missing():
    def boom(*a, **k):
        raise LookupError("no prod version")
    eng = SwingEngine(_model_loader=boom, _universe=lambda limit=None: ["AAA"],
...
# The new swing artifacts list ALL forecast cols (tsfm + kronos + chronos + ens).
_FORECAST_ORDER = list(SWING_FEATURE_ORDER) + list(FORECAST_FEATURES)
...
def _write_forecast_cache(cache_dir, symbols):
    ...
    frame(["chronos_fwd_ret", "chronos_uncert"]).to_parquet(
        cache_dir / "swing_chronos.parquet", index=False)
...
def test_forecast_cols_merged_from_cache(tmp_path, monkeypatch):
    ...
    def fake_loader(symbols, start, end, freq="eod", provider=None):
        frames = []
        for s in symbols:
            df = _ramp_ohlcv({"AAA": 0.0, "BBB": 5.0, "CCC": 10.0}.get(s, 3.0), idx)
            df["symbol"] = s
            frames.append(df)
        return pd.concat(frames, ignore_index=True)

    cap = _CapturingNanBooster()
    eng = SwingEngine(_booster=cap, _feature_order=list(_FORECAST_ORDER),
                      _universe=lambda limit=None: syms, _load_ohlcv=fake_loader)
    sigs = eng.run(top_n=3)
    assert len(sigs) == 3
    ...
    chronos_seen = sorted(round(float(df["chronos_fwd_ret"].iloc[0]), 6) for df in cap.seen)
    assert chronos_seen == [1.009, 2.009, 3.009]
    for df in cap.seen:
        row = df.iloc[0]
...
def test_degraded_but_serving_when_cache_absent(tmp_path, monkeypatch, caplog):
    ...
    for df in cap.seen:
        assert df[list(FORECAST_FEATURES)].isna().all().all()
